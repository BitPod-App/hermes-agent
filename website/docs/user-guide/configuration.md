...
`hard_stop_enabled` explicitly enables hard stops on every platform. When it remains `false`, `non_interactive_hard_stop_enabled` still enables them for unattended gateway/cron-style platforms and scripted `hermes -z` invocations, while preserving warning-only behavior for interactive CLI, TUI, Desktop, ACP, subagents, and `api_server` runs (supervised task loops with a live parent or client). One-shot mode retains the `cli` toolset and session source; its unattended execution posture alone arms the hard stop. Set `non_interactive_hard_stop_enabled: false` to opt an unattended deployment out. See also [Docker / unattended deployments](docker.md).
...
- **A halt ends the turn, not a persistent session.** The agent replies with which guardrail fired and why; replying "continue" resumes with fresh per-turn counters. In scripted `hermes -z` mode, the explanation is still printed, the run is recorded as failed, and the process exits `2` because no operator is present to continue.
